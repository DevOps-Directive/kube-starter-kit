...
git log --oneline -10 -- kubernetes/src/infrastructure/ingress-nginx/
...
```yaml
chartVersions:
  ingress-nginx: "4.12.2"  # Previous version
```
...
<Steps>
  <Step title="Enable versioning recovery">
    ...
    aws s3api list-object-versions \
      ...
      --prefix terraform/staging/us-east-2/eks/
    ```
  </Step>
  ...
  <Step title="Restore previous state">
    ...
    aws s3api copy-object \
      --bucket ksk-gbl-infra-bootstrap-state \
      --copy-source "ksk-gbl-infra-bootstrap-state/terraform/staging/us-east-2/eks/terraform.tfstate?versionId=<version-id>" \
      --key terraform/staging/us-east-2/eks/terraform.tfstate
    ```
  </Step>
  ...
  <Step title="Verify and plan">
    ```bash
    cd terraform
    terramate run --tags staging:us-east-2:eks -- terraform init -reconfigure
