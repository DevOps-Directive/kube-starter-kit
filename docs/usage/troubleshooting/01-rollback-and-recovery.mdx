...
<Steps>
  ...
  <Step title="Enable versioning recovery">
    ...
    aws s3api list-object-versions \
      --bucket ${STATE_BUCKET} \
      --prefix terraform/${STAGE}/${REGION}/eks/
    ```
  </Step>
  ...
  <Step title="Restore previous state">
    ...
    aws s3api copy-object \
      --bucket ${STATE_BUCKET} \
      --copy-source "${STATE_BUCKET}/terraform/${STAGE}/${REGION}/eks/terraform.tfstate?versionId=<version-id>" \
      --key terraform/${STAGE}/${REGION}/eks/terraform.tfstate
    ```
  </Step>
  ...
  <Step title="Verify and plan">
    ```bash
    cd terraform
    terramate run --tags ${STAGE}:${REGION}:eks -- terraform init -reconfigure
