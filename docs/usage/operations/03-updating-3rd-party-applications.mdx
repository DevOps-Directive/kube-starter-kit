...
<Steps>
  ...
  <Step title="Update the version">
    ...
    For environment-specific versions, edit `values.staging.yaml` or `values.production.yaml`.
  </Step>
...
<Steps>
  <Step title="Edit the values file">
    ...
    Example—increase controller replicas:
    ```yaml
    ingress-nginx:
      controller:
        replicaCount: 3
...
Many components need additional resources beyond the upstream chart (ClusterIssuers, StorageClasses, etc.). Add these in the wrapper chart's `templates/` directory:
...
4. **Plan for downtime** if the component is critical
...
rm -rf kubernetes/src/infrastructure/*/charts/
rm -f kubernetes/src/infrastructure/*/Chart.lock
...
### ArgoCD sync fails after upgrade
...
2. View sync errors:
   ```bash
   argocd app sync ingress-nginx --dry-run
   ```

3. Common issues:
   - CRD version mismatch (update CRDs first)
   - Removed/renamed values (check changelog)
   - Resource conflicts (prune may be needed)
