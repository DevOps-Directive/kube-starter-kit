...
Once the manual roles exist, Terraform can import and manage them. The `account-bootstrapping` module includes an `import` block to bring the manually-created role into Terraform state.

#### How Import Works

The module uses the same naming convention for both creating and importing the role:

```hcl
# In terraform/modules/account-bootstrapping/main.tf
...
The `module.this.id` value (from the null-label context) generates the same name used when you manually created the role (e.g., `<NAMESPACE>-gbl-staging-bootstrap-admin`).
