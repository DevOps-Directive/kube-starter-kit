...
<Warning>
ArgoCD may show errors on the first apply (e.g., "namespace not found" or "CRD not found"). This is expected - simply run the apply command again to create the remaining resources.
</Warning>
...
<Steps>
  ...
  <Step title="Apply the app-of-apps">
    ...
    ArgoCD will then automatically deploy all infrastructure components (cert-manager, external-secrets, external-dns, ingress-nginx, etc.) and services.
  </Step>
</Steps>
...
<Steps>
  ...
  <Step title="Verify ArgoCD can sync">
    ...
    - Infrastructure components deploy via the `infrastructure-app-of-apps`
...
| **ingress-nginx** | ArgoCD | Ingress controller for routing external traffic |
