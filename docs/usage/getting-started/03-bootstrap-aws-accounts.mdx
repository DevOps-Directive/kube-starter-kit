...
<Steps>
  ...
  <Step title="Create IAM roles in target accounts">
    Each target account needs an IAM role that can be assumed from the Infrastructure account. This role has a trust policy that allows the Infrastructure account's SSO admin role (and later, the GitHub OIDC role) to assume it.
  ...
  <Step title="Apply Terraform bootstrapping stacks">
    ...
    - Configure the S3 state bucket with versioning
    - Update target account IAM roles to also trust the GitHub OIDC role
