...
<Steps>
  ...
  <Step title="Initialize and deploy networking">
    ...
    terramate run --tags staging:${REGION}:networking -- terraform init

    # Preview changes
    terramate run --tags staging:${REGION}:networking -- terraform plan

    # Apply
    terramate run --tags staging:${REGION}:networking -- terraform apply
    ```
  </Step>
  ...
  <Step title="Deploy EKS">
    ...
    terramate run --tags staging:${REGION}:eks -- terraform init

    # Preview and apply
    terramate run --tags staging:${REGION}:eks -- terraform plan
    terramate run --tags staging:${REGION}:eks -- terraform apply
    ```
  ...
  <Step title="Deploy example application resources (optional)">
    ...
    ```bash
    terramate run --tags staging:${REGION}:services -- terraform init
    terramate run --tags staging:${REGION}:services -- terraform apply
    ```
  </Step>
...
<Steps>
  <Step title="Check Terraform outputs">
    ```bash
    terramate run --tags staging:${REGION}:eks -- terraform output
    ```
